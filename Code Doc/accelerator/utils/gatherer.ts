...
export class Gatherer<Request, Response> {
  ...
  constructor(
    ...
    this.topic = new Topic<{ id: number; request: Request }>(
      name,
      version,
      false
    );
  ...
  destroy() {
    this.logIfDebug(
      this.topic.name,
      `Gatherer ${this.topic.name}: ${this.topic.version} destroyed`
    );
  ...
  async gather(request: Request): Promise<Response[]> {
    ...
    const promises = window["@onecx/accelerator"].gatherer.promises[
      id
    ] as Promise<Response>[];
    delete window["@onecx/accelerator"].gatherer.promises[id];
  ...
  private logReceivedIfDebug(
    name: string,
    version: number,
    m: { id: number; request: Request }
  ) {
    this.logIfDebug(
      name,
      "Gatherer " + name + ": " + version + " received request " + m.request
    );
  }
