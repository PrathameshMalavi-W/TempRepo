...
FIle => onecx-shell-ui  > src >  app > shell > components > slot-group > 


File : slot-group.component.ts
```ts
import { CommonModule } from '@angular/common'
...
File : slot-group.component.html
```html
<ocx-slot
...
File : slot-group.component.spec.ts
Below => end 


********************************************************************************************************************************

FIle => onecx-shell-ui  > src >  app > shell > components > portal-viewport > 



File : portal-viewport.component.html
```html
...
File : portal-viewport.componentscss
```scss
.onecx-viewport-wrapper {
  display: flex;
  flex-direction: column;
  height: 100vh;
}
...
.onecx-body-wrapper {
  ...
  .onecx-body-start,
  .onecx-body-end {
    // Shared max-width between header start and body start
    max-width: 20rem;
    background-color: var(--menu-bg-color);
    box-shadow: 1px 1px 0 0 var(--divider-color, #0003);
    overflow-x: hidden;
    overflow-y: auto;
    flex-grow: 0;
    flex-shrink: 0;
...
.onecx-header {
  ...
  // Use flexbox to layout the three sections
  display: flex;
  justify-content: space-between;
  align-items: center;
  row-gap: 1rem;

  // Slots should take full height of the header
  ocx-slot {
    height: 100%;
  }

  // Each section (start, center, end) takes full height and uses flexbox for internal layout
  .onecx-header-start,
  .onecx-header-center,
  .onecx-header-end {
    height: 100%;
  ...
  .onecx-header-start {
    // Not grow to keep the size of the left section static until not possible
    flex-grow: 0;
    // Shrink if necessary
    flex-shrink: 1;
    ...
    .onecx-menu-button {
      ...
      cursor: pointer;
      border-radius: 50%;
      color: var(--topbar-menu-button-text-color, #ffffff);
      background-color: var(--topbar-menu-button-bg-color);
  ...
  .onecx-header-center {
    // Allow to grow to take all remaining space
    flex-grow: 1;
    // Shrink if necessary
    flex-shrink: 1;
  ...
  .onecx-header-end {
    ...
    // Shrink if necessary
    flex-shrink: 1;
    // Shared max-width between header end and body end
    max-width: 25rem;

    .onecx_header_end_actions_right {
      height: 100%;
      margin-right: 0.75rem;

      .onecx_header_end_actions_right_items {
        display: flex;
        flex-direction: row;
        flex-wrap: wrap;
        justify-content: center;
...
.onecx-header:not(:has(ocx-slot:not(:empty))) {
  height: 0;
}

// Necessary to visually hide the extensions slot.
// Using [hidden]=true causes the slot to be present in DOM, but not rendered visibly
// which causes <p-toast> not to be rendered and toast messages not being displayed
.extensions-slot {
  width: 0 !important;
  height: 0 !important;
...
File : portal-viewport.component.ts
```ts
...
FIle => onecx-shell-ui  > src >  app > shell > components > global-error > 
...
File : app-loading-spinner.component.html
```html
...
File : app-loading-spinner.componentscss
```scss
...
File : app-loading-spinner.component.ts
```ts
...
FIle => onecx-shell-ui  > src >  app > not-found > 


File :  not-found-page.module.ts
```ts

```


File : components/components/not-found-page =>  not-found-page.component.ts
```ts
...
FIle => onecx-shell-ui  > src >  app > init-error > 


File :  initialization-error-page.module.ts
```ts
import { NgModule } from '@angular/core'
import { CommonModule } from '@angular/common'
import { RouterModule } from '@angular/router'
...
File : components/components/initialization-error-page =>  initialization-error-page.component.ts
```ts
import { Component } from '@angular/core'
...
File : components/components/initialization-error-page =>  initialization-error-page.component.html
```html
...
File : components/components/initialization-error-page =>  initialization-error-page.component.spec.ts
```ts
...
FIle => onecx-shell-ui  > src >  app > error > 
...
File : components/components/error-page =>  error-page.component.ts
```ts
import { Component } from '@angular/core'
...
FIle => onecx-shell-ui  > src >  app > home > 
...
File : components/components/home =>  home.component.ts
```ts
import { Component } from '@angular/core'
...
File : components/components/home =>  home.component.html
```html
...
File : components/components/welcome-message-compoenent =>  welcome-message.component.ts
```ts
import { Component } from '@angular/core'
...
File : components/components/welcome-message-compoenent  =>  welcome-message.component.html
```html
