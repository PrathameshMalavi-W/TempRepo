...
export class KeycloakAuthService implements AuthService {
  ...
  public async init(config?: Record<string, unknown>): Promise<boolean> {
    ...
    const timeSkew = await this.getNumericConfig(CONFIG_KEY.KEYCLOAK_TIME_SKEW)
  ...
  private async setupEventListener() {
    if (this.keycloak) {
      ...
      const authRefreshErrorEnabled =
        (await this.configService.getProperty(CONFIG_KEY.KEYCLOAK_ON_AUTH_REFRESH_ERROR_ENABLED)) === 'true'
      this.keycloak.onAuthRefreshError = () => {
        this.updateLocalStorage()
        if (authRefreshErrorEnabled) {
          this.logger.info('Auth refresh error - triggering login')
          void this.keycloak?.login(this.config)
        }
      }
      ...
      const tokenExpiredEnabled =
        (await this.configService.getProperty(CONFIG_KEY.KEYCLOAK_ON_TOKEN_EXPIRED_ENABLED)) === 'true'
      this.keycloak.onTokenExpired = () => {
        this.updateLocalStorage()
        if (tokenExpiredEnabled) {
          this.logger.info('Token expired - triggering token refresh')
          void this.updateTokenSemaphore
            .use(async () => {
            ...
            .catch((error) => {
              this.logger.warn('Token refresh after expiration failed', error)
              return false
            })
        }
      }
  ...
  async updateTokenIfNeeded(): Promise<boolean> {
    return this.updateTokenSemaphore.use(async () => {
      ...
      if (!this.keycloak.authenticated) {
        await this.keycloak.login(this.config)
        return false
      }
  ...
  private async updateToken(): Promise<boolean> {
    ...
    const minValidity = await this.getNumericConfig(CONFIG_KEY.KEYCLOAK_UPDATE_TOKEN_MIN_VALIDITY)
    return this.keycloak.updateToken(minValidity)
  }

  private async getNumericConfig(key: CONFIG_KEY): Promise<number | undefined> {
    const configuredValue = await this.configService.getProperty(key)
    ...
    if (Number.isNaN(parsedValue)) {
      this.logger.warn(`Ignoring invalid numeric config value for ${key}: ${configuredValue}`)
      return undefined
    }
