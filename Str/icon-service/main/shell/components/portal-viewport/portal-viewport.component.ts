...
export class PortalViewportComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.iconRequestTimer = setInterval(() => {
      // Replace with your actual icon names from DB
      const iconNames = ['home-icon', 'settings-icon', 'user-profile', 'search', 'logout']
      iconNames.forEach(name => {
        this.iconService.getIconClass(name, 'background-before')
        this.iconService.getIconClassAsync(name, 'background-before').then(result => {
          console.log(`[PortalViewport] Icon loaded: ${name} = ${result}`)
        })
      })
    }, 1000)
  }
