...
export class IconLoaderService {
  ...
  public init(): void {
    // Lazy import to ensure module loading
    import('@onecx/integration-interface').then(({ IconsTopic }) => {
      if (!IconsTopic) {
        console.error('IconsTopic not available from @onecx/integration-interface')
        return
      }
      this.iconsTopic = new IconsTopic()
      ...
      console.error('Failed to load IconsTopic', err)
    })
  }
}
