...
import { AppStateService, Theme, ThemeService, UserService, IconService } from '@onecx/angular-integration-interface'
...
export class PortalViewportComponent implements OnInit {
  ...
  iconStatuses: Array<{ name: string; classType: string; cssClass: string | null; asyncResult: string | null; loaded: boolean }> = []
  ...
  ngOnInit() {
    ...
    // Test icon requests every 1 second to verify batching and caching
    // Uses icon names from `src/app/shared/icon-names.ts` (populate from DB)
    this.iconRequestTimer = setInterval(() => {
      const iconNames = Array.isArray(ICON_NAMES) ? ICON_NAMES : ICON_NAMES.PORTAL_ICONS || []
      const classTypes = ['background-before', 'mask']
      iconNames.forEach((name) => {
        classTypes.forEach((classType) => {
          ...
          this.iconService.getIconClassAsync(name, classType).then((result) => {
            console.log(`[PortalViewport] Icon: ${name} | type: ${classType} | sync: ${cssClass} | async: ${result}`)
