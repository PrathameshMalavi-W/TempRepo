...
JS engine loaded the file
    Function justdeclared { 
      ...
      slotInitializer
      permissionProxyInitialize
      configurationServiceInitializer
      imageRepositoryServiceInitializer
      checkIfReactRouterInitialization
      _constructCurrentURL
      urlChangeListenerInitializer
      apply
      shareMfContainer
    }
Overrode history.pushState
Overrode history.replaceState
Angular not started yet


Angular sees AppModule
...
import { IconLoaderService } from './shell/services/icon-loader.service'
...
@NgModule({
  ...
  providers: [
    ...
    provideAppInitializer(() => {
      return inject(IconLoaderService).init()
    }),
